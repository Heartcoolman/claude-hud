...
| `elementOrder` | string[] | `["project","context","addedDirs","proxy","promptCache","memory","environment","tools","agents","todos","sessionTime"]` | Expanded-mode element order. Omit entries to hide them in expanded mode. This fork's default omits `usage` in favor of `proxy` (ReClaude); add `"usage"` back to surface Anthropic-native rate limits. `project` and `context` are adjacent so the default `mergeGroups` can fuse them onto one line. Existing configs keep their explicit order until updated. |
| `display.mergeGroups` | string[][] | `[["project","context"]]` | Expanded-mode groups that should share a line when adjacent. Set `[]` to disable merged lines. |
