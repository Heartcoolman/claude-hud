...
let cachedWinKey: Buffer | null = null;
...
function macCookiesPath(): string {
  return path.join(
    os.homedir(),
    'Library',
    'Application Support',
...
async function readMacEncryptedHex(
  cookiesPath: string,
  hostFragment: string,
  cookieName: string,
): Promise<string | null> {
...
function decryptMacCbc(ciphertext: Buffer, key: Buffer): string | null {
  try {
...
function winChromeUserDataDir(): string | null {
  const local = process.env.LOCALAPPDATA;
  if (!local) return null;
  return path.join(local, 'Google', 'Chrome', 'User Data');
}

function winCookiesPath(userData: string): string | null {
  const newer = path.join(userData, 'Default', 'Network', 'Cookies');
  if (fs.existsSync(newer)) return newer;
...
async function getWinChromeKey(userData: string): Promise<Buffer | null> {
  if (cachedWinKey) return cachedWinKey;
  ...
  if (enc.length <= 5 || enc.subarray(0, 5).toString('utf8') !== 'DPAPI') return null;
...
interface WinCookieRow {
  hostKey: string;
  encrypted: string;
}

async function readWinCookieRow(
  cookiesPath: string,
  hostFragment: string,
  cookieName: string,
): Promise<WinCookieRow | null> {
  const safeHost = hostFragment.replace(/'/g, "''");
  const safeName = cookieName.replace(/'/g, "''");
  ...
  if (!out) return null;
  try {
    const parsed = JSON.parse(out) as { hostKey?: unknown; encrypted?: unknown };
    if (typeof parsed.hostKey !== 'string' || typeof parsed.encrypted !== 'string') return null;
...
function decryptWinGcm(blob: Buffer, key: Buffer, hostKey: string): string | null {
...
async function readWinCookie(
  ...
  const ver = encrypted.subarray(0, 3).toString('utf8');
  if (ver !== 'v10') return null; // v20 (app-bound) not supported
...
export async function readChromeCookie(
  ...
  if (process.platform === 'darwin') return readMacCookie(hostFragment, cookieName);
  if (process.platform === 'win32') return readWinCookie(hostFragment, cookieName);
  return null;
}
