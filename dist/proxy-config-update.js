...
export function updateReclaudeCookieInConfig(configPath, rcSidValue) {
    let raw;
    try {
    ...
    if (!isObject(parsed.display))
        parsed.display = {};
    const display = parsed.display;
    if (!isObject(display.reclaude))
        display.reclaude = {};
    const reclaude = display.reclaude;
    const newCookie = rcSidValue.startsWith('rc_sid=') ? rcSidValue : `rc_sid=${rcSidValue}`;
    if (reclaude.cookie === newCookie)
