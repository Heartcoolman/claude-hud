...
function parseArgs(argv) {
    ...
    return {
        ...
        autoRefresh: ar === 'chrome' || ar === 'credentials' || ar === 'chrome+credentials' ? ar : 'off',
        email: process.env.CLAUDE_HUD_RECLAUDE_EMAIL ?? out['email'] ?? '',
        passwordKeychainService: process.env.CLAUDE_HUD_RECLAUDE_KEYCHAIN ?? out['keychain-service'] ?? '',
    };
}
...
function sentinelPath(cachePath) {
    return `${cachePath}.error`;
}
function writeSentinel(cachePath, code) {
    try {
...
function clearSentinel(cachePath) {
    try {
        fs.unlinkSync(sentinelPath(cachePath));
    }
    catch { /* ignore */ }
}
async function fetchQuota(url, cookie, timeoutMs) {
    try {
...
function writeCacheAtomically(cachePath, body) {
    fs.mkdirSync(path.dirname(cachePath), { recursive: true });
...
function normaliseCookie(rcSid) {
    return rcSid.startsWith('rc_sid=') ? rcSid : `rc_sid=${rcSid}`;
}
async function tryTier2Chrome(args) {
    if (args.autoRefresh !== 'chrome' && args.autoRefresh !== 'chrome+credentials')
        return null;
    const fresh = await readChromeCookie('reclaude.ai', 'rc_sid');
    if (!fresh)
        return null;
    const candidate = normaliseCookie(fresh);
    if (candidate === args.cookie)
        return null; // same as what failed; skip
    return candidate;
}
async function tryTier3Login(args) {
    if (args.autoRefresh !== 'credentials' && args.autoRefresh !== 'chrome+credentials')
        return null;
    if (!args.email)
        return null;
...
void main();
//# sourceMappingURL=proxy-usage-fetcher.js.map