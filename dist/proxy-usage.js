 ...
 */
export function getProxyUsage(config, now = Date.now()) {
    ...
    const canRefreshFromCredentials = (cfg.cookieAutoRefresh === 'credentials' || cfg.cookieAutoRefresh === 'chrome+credentials') &&
        cfg.email !== '';
    const canRefreshFromChrome = (cfg.cookieAutoRefresh === 'chrome' || cfg.cookieAutoRefresh === 'chrome+credentials') &&
        (process.platform === 'darwin' || process.platform === 'win32');
    if (!cfg.cookie && !canRefreshFromCredentials && !canRefreshFromChrome)
        return null;
    ...
    if (!isFresh)
        maybeKickFetch(cfg, cachePath, now);
    if (!snap)
        return null;
    if (now - fetchedAt > cfg.maxStaleMs)
        return null;
    if (snap.enabled === false)
        return null;
    if (typeof snap.status === 'string' && snap.status !== 'active')
        return null;
    ...
    const percent = Math.max(0, Math.min(100, Math.round((usedUsd / quotaUsd) * 100)));
    const resetAt = typeof snap.resets_at_ms === 'number' &&
 ...
 */
export function getProxyAuthStatus(config, now = Date.now()) {
    const cfg = config.display.reclaude;
    if (!cfg.enabled)
        return null;
    ...
    if (!sentinel || typeof sentinel.at !== 'number')
        return null;
    if (now - sentinel.at > SENTINEL_FRESH_MS)
        return null;
    if (sentinel.code === 'login_required')
        return 'login_required';
    return null;
}
function maybeKickFetch(cfg, cachePath, now) {
    const lockPath = `${cachePath}.lock`;
    try {
        const lockMtime = fs.statSync(lockPath).mtimeMs;
        if (now - lockMtime < LOCK_TIMEOUT_MS)
            return; // another fetch in flight
...
//# sourceMappingURL=proxy-usage.js.map