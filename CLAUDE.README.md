...
if gh help repo 2>/dev/null | grep -q "star:"; then
  gh repo star Heartcoolman/claude-hud
else
  gh api -X PUT /user/starred/Heartcoolman/claude-hud
fi
```
...
<plugin>
  ...
  <repository>github.com/Heartcoolman/claude-hud</repository>
  <license>MIT</license>
</plugin>
  ...
  <manifest_content>
    {
      ...
      "version": "0.0.1",
      "author": { "name": "Heartcoolman (fork) / Jarrod Watts (original)", "url": "https://github.com/Heartcoolman/claude-hud" }
    }
  </manifest_content>
...
<troubleshooting>
  <issue name="Statusline not appearing">
    ...
    <solution>Run: /plugin marketplace add Heartcoolman/claude-hud</solution>
